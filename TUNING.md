...
| `STRUCTURAL_COMMANDS` | `lit/arxiv_fetch.py` | 19 names | The LaTeX commands whose redefinition the macro collector drops, so the name keeps the meaning the conversion reads it by. It holds the sectioning commands, `caption`, `label`, `ref`, `cite`, `bibitem`, `includegraphics`, `input`, `include`, `begin` and `end`. A name added to it keeps a paper from hiding that structure behind its own wrapper. A name taken out of it lets a paper's own definition win, and a paper that wraps its headings then arrives as one undivided chapter. The list is a judgement: it names what the conversion scans for, and nothing else. |
