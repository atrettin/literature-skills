...
`litdb init` starts a collection where it will live: `literature/` in the
project by default, or a shared place you name with `LITERATURE_ROOT`. It
creates the directory with its index and makes git ignore it — the papers are
copyrighted and must never be committed — and it refuses if one already exists.

```bash
litdb init --flavor vscode
```

The `--flavor` determines the Markdown flavor that is used to render the 
literature and the reports. Set it according to the application you intend
to use to view them. Two options are available:

| Flavor | What opens it |
|---|---|
| `vscode` (the default) | the Markdown preview built into VS Code, and any KaTeX-capable reader |
| `obsidian` | Obsidian, whose block addresses and links it writes |

They differ only in the shape of the anchors and links, and in how display
maths is wrapped for KaTeX and for MathJax. The entire literature and the
reports generated by these skills can be re-rendered into a different Markdown
flavor with a single command:

```bash
litdb render --flavor obsidian
```

[docs/rendering.md](docs/rendering.md) has the full table of what differs.

Note: The rendered markdown files are only for human-friendly display. The actual content
of the literature is stored in JSONL files that are independent from the rendering.
